...
function setupCLI() {
  ...
  program
    ...
    .description('Manage SSH keys (actions: add-to-agent, remove-from-agent, list, validate)')
    .action(() => {});
...
function parseArguments(argv = process.argv) {
  ...
  if (argv.length === 2) {
    return { command: 'show-quick-help', args: {}, options: {} };
  }
  
  // Check for -i flag early
  if (argv.includes('-i') || argv.includes('--interactive')) {
    return { command: 'interactive', args: {}, options: { interactive: true } };
  }
  ...
  if (globalOptions.interactive && !result) {
    return { command: 'interactive', args: {}, options: globalOptions };
  }
