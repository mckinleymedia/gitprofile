...
const validators = {
  ...
  accountName(name) {
    ...
    if (trimmed.length > 50) {
      throw new ValidationError('Account name is too long (max 50 characters)');
    }
    
    if (!/^[a-zA-Z0-9_-]+$/.test(trimmed)) {
      throw new ValidationError('Account name can only contain letters, numbers, hyphens, and underscores');
    }
