...
`;
    ...
    try {
      ...
      if (currentConfig.includes(`Host ${hostAlias}`)) {
        ...
        const regex = new RegExp(`# GitSwitch - ${profileName}[\\s\\S]*?(?=\\n# |\\n\\n|$)`, 'g');
  ...
  removeFromSSHConfig(profileName) {
    const configPath = path.join(this.sshDir, 'config');
