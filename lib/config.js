...
class ConfigManager {
  constructor() {
    ...
    // Check for old config file for backward compatibility
    this.oldConfigPath = path.join(
      process.env.HOME || process.env.USERPROFILE,
      '.gitswitch.json'
    );
  ...
  load() {
    try {
      ...
      if (!fs.existsSync(this.configPath) && fs.existsSync(this.oldConfigPath)) {
        // Migrate old config to new location
        const oldData = fs.readFileSync(this.oldConfigPath, 'utf8');
        fs.writeFileSync(this.configPath, oldData);
        console.log('Migrated configuration from .gitswitch.json to .gitprofile.json');
      }
      ...
      if (fs.existsSync(this.configPath)) {
        ...
        // Migration: Convert accounts to profiles
        if (config.accounts && !config.profiles) {
          config.profiles = config.accounts;
